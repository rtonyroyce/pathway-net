...
# ─────────────────────────────────────────────
#   GLOBAL CSS  —  Light Minimal Sci-Fi Theme
# ─────────────────────────────────────────────
st.markdown("""
<style>
...
section[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #eef0f5 0%, #e8eaf2 100%) !important;
    border-right: 1px solid #d0d5e8;
}
hr { border-color: #d0d5e8 !important; }
...
div[data-testid="stButton"] > button:hover {
    background: linear-gradient(135deg, #1d55d4, #1749bb);
...
.stProgress > div > div > div > div {
    background: linear-gradient(90deg, #2563eb, #3b82f6, #059669) !important;
}
...
.pulse-dot {
    ...
    border-radius: 50%;
    background: #059669;
    box-shadow: 0 0 7px rgba(5,150,105,0.6);
    animation: pulseAnim 2s ease-in-out infinite;
    flex-shrink: 0;
}
@keyframes pulseAnim {
    0%,100% { opacity:1; box-shadow: 0 0 7px rgba(5,150,105,0.6); }
...
.section-hdr-line {
    flex: 1;
    height: 1px;
    background: linear-gradient(90deg, #d0d5e8, transparent);
}
.section-hdr-text {
    font-family: 'JetBrains Mono', monospace;
    font-size: 0.68em;
...
.compound-no-data-tag {
    ...
    border-color: rgba(139,154,184,0.25);
    color: #8b9ab8;
}
.compound-name {
    font-family: 'JetBrains Mono', monospace;
...
.verdict-footer span b { color: #4b6cac; }
...
/* ── LEARN PAGE ─────────────────────────── */
.learn-hero {
    background: linear-gradient(135deg, #ffffff 0%, #f0f4ff 60%, #eef2ff 100%);
    border: 1px solid #dde3f0;
    border-radius: 16px;
...
.learn-body strong { color: #1a2035; }
.learn-body em { color: #2563eb; font-style: normal; font-weight: 500; }
.learn-analogy {
...
.learn-callout strong { color: #065f46; }
.learn-warn {
...
.credits-data-pill span { color: #2563eb; display:block; font-size:0.88em; margin-top:3px; }
...
.res-body strong { color: #1a2035; }
.res-body em { color: #2563eb; font-style: normal; font-weight: 600; }
...
.res-highlight strong { color: #1a2035; }
.res-formula {
    background: #f4f5f7;
...
def render_compound_card(drug_name: str, desc_dict: dict, dr_df):
    ...
    if has_gdsc:
        ...
        MP = (
            ...
            'font-family:"JetBrains Mono",monospace;color:#4b6cac;'
            'display:inline-block;'
        )
        VS = 'color:#2563eb;'
        ...
        CP = (
            'display:inline-block;background:rgba(124,58,237,0.06);'
            'border:1px solid rgba(124,58,237,0.18);color:#7c3aed;border-radius:6px;'
    ...
    CARD_BORDER = "#2563eb" if has_desc else "#c7d7fb"

    CARD_S = (
        f'background:#ffffff;'
        f'border:1px solid #dde3f0;border-left:4px solid {CARD_BORDER};'
        f'border-radius:14px;padding:28px 30px 24px 30px;margin-bottom:28px;'
        f'box-shadow:0 2px 12px rgba(37,99,235,0.05);'
    )
    TAG_S = (
        'display:inline-block;'
        + ('background:rgba(37,99,235,0.08);border:1px solid rgba(37,99,235,0.2);color:#2563eb;'
           if has_desc else
           'background:rgba(139,154,184,0.1);border:1px solid rgba(139,154,184,0.25);color:#8b9ab8;')
    ...
    NAME_S = (
        ...
        'color:#1a2035;margin:12px 0 14px 0;line-height:1.3;'
    )
    DESC_S = (
        'font-family:"Inter",sans-serif;font-size:0.9em;line-height:1.85;margin:0;'
        + ('color:#3d4f72;' if has_desc else 'color:#8b9ab8;font-style:italic;')
    )
    DIV_S  = 'height:1px;background:#e8eaf2;margin:20px 0;'
    CL_S   = (
        'font-family:"JetBrains Mono",monospace;font-size:0.63em;letter-spacing:2.5px;'
        'text-transform:uppercase;color:#8b9ab8;margin-bottom:10px;'
    )
...
PLOT_CFG = dict(
    ...
    plot_bgcolor='rgba(244,245,247,0.6)',
    font=dict(family='JetBrains Mono', color='#4b6cac', size=11),
)
...
""", unsafe_allow_html=True)
    ...
    with col_r:
        ...
        fig_scat.add_trace(go.Scatter(
            ...
            marker=dict(
                color=abs_res,
                colorscale=[[0, '#059669'], [0.4, '#fbbf24'], [1.0, '#ef4444']],
                size=9, opacity=0.82,
        ...
        fig_scat.add_trace(go.Scatter(
            ...
            line=dict(color='rgba(0,0,0,0.12)', width=1.5, dash='dot'),
        ...
        fig_scat.update_layout(
            **PLOT_CFG,
            title=dict(
                ...
                font=dict(size=11, color='#4b6cac')
            ),
            height=380,
            margin=dict(l=30, r=70, t=55, b=30),
            xaxis=dict(title='Actual ln IC50',   gridcolor='#e8eaf2', zeroline=False),
            yaxis=dict(title='Predicted ln IC50', gridcolor='#e8eaf2', zeroline=False),
        ...
        fig_box.update_layout(
            **PLOT_CFG,
            ...
            xaxis=dict(
                ...
                gridcolor='#e8eaf2', zeroline=False,
                title_font=dict(size=11)
            ),
    ...
    fig_resist.add_trace(go.Bar(
        ...
        y=pw_labels[::-1],
        orientation='h',
        marker=dict(color=bar_clr[::-1], opacity=0.82,
                    line=dict(color='rgba(0,0,0,0.04)', width=1)),
        text=[f"{v:+.3f}" for v in top_scores[::-1]],
        textposition='outside',
        textfont=dict(size=10, color='#4b6cac'),
        ...
        **PLOT_CFG,
        ...
        annotations=[
            ...
            dict(x=min(top_scores) * 0.55, y=-0.065, xref='x', yref='paper',
                 ...
                 font=dict(color='#2563eb', size=9, family='JetBrains Mono')),
        ]
    )
    ...
    for col, label, val, sub, color in [
        (p1, "Baseline ln IC50",    f"{baseline_pred:.3f}", "Pre-perturbation (most resistant cell)", "#1a2035"),
        (p2, "Post-CRISPR ln IC50", f"{cf_pred:.3f}",       "After pathway knockout",                 "#1a2035"),
        (p3, "Sensitivity Δ",       f"{delta:+.4f}",        "Positive = sensitised to drug",          c_border),
    ]:
        with col:
            st.markdown(f"""
<div class="kpi-outer" style="{'border:1px solid ' + c_border + '44;' if label == 'Sensitivity Δ' else ''}">
  <div class="kpi-label">{label}</div>
  <div class="kpi-value" style="color:{color};">{val}</div>
  <div class="kpi-sub">{sub}</div>
</div>
...
<div class="verdict-wrap"
     style="border:1px solid {c_border}33; border-left:4px solid {c_border}; margin-top:20px;">
  <div style="display:flex; align-items:flex-start; gap:16px;">
    ...
    <div style="flex:1;">
      ...
      <div style="font-family:'JetBrains Mono',monospace; font-size:0.62em; letter-spacing:3px;
                  text-transform:uppercase; color:#8b9ab8; margin-bottom:6px;">Verdict</div>
